...
export default function WhatsAppButton({
	...
	message = `שלום ${siteConfig.storeName}, אשמח לקבל פרטים 😊`,
	size = "md",
	className = "",
}: WhatsAppButtonProps) {
	...
	return (
		<a
			...
			className={`inline-flex items-center justify-center gap-2 rounded-full bg-[var(--color-accent)] text-white shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all focus-visible:ring-2 focus-visible:ring-white/80 ${sizes[size]} ${className}`}
			aria-label={`שליחת הודעה ב-WhatsApp ל${siteConfig.storeName}`}
		>
			<svg
